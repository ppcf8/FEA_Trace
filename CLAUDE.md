...
- **`app/core/`** — Business logic: `models.py` (FEAProject persistence), `session.py` (multi-entity sessions), `migration.py` (schema versioning), `settings.py` (user presets persistence).
...
**Schema history:** `1.0.0` had a `RepresentationRecord` level between Version and Iteration. `2.0.0` merges it into Iteration. `2.1.0` removes the redundant `design_changes` field from `IterationRecord`. `2.2.0` renames `VersionRecord.intent` → `description`. `2.3.0` adds `promoted_at: str` to `VersionRecord`. `2.4.0` adds `status: IterationStatus` and `notes: list[str]` to `IterationRecord`. `2.5.0` moves `promoted_at` from `VersionRecord` to `IterationRecord` and adds `IterationStatus.PRODUCTION`; auto-migration infers iteration PRODUCTION status from existing `is_production` run flags. `2.6.0` adds `CommunicationRecord` dataclass and `VersionRecord.communications: list[CommunicationRecord]`; auto-migration adds `communications: []` to existing versions. `2.7.0` renames `CommunicationRecord.eml_filename: str` → `eml_filenames: list[str]`; auto-migration converts the old scalar to a one-element list. `2.8.0` **(current)** adds `SourceComponentRecord` dataclass and `VersionRecord.source_components: list[SourceComponentRecord]`; auto-migration adds `source_components: []` to all existing versions. All migrations `1.0.0 → 2.8.0` are registered in `app/core/migration.py`; major migrations require user confirmation, minor ones are auto-applied.

### Status State Machines

**Iteration:** WIP / PRODUCTION / DEPRECATED. `WIP → PRODUCTION` via `promote_iteration_to_production()` (requires no WIP runs); `WIP → DEPRECATED` requires a mandatory reason via `RevertReasonDialog(mode="deprecate")`; `PRODUCTION → WIP` / `DEPRECATED → WIP` (revert requires a reason via `RevertReasonDialog`). Reverting from PRODUCTION clears `promoted_at` and all `is_production` flags in the iteration. PRODUCTION iterations: Edit disabled, New Run disabled, Delete Run disabled, all RunFrame edits locked (`lock = is_production or not is_version_wip or is_iter_production`). Sidebar shows `● Production` (green) or `● Deprecated` (grey) suffix.
...
**File menu** uses `CTkMenuBar` + `CustomDropdownMenu` from the `CTkMenuBar` package (see `requirements.txt`). Menus: File (New Entity, Open Entity, New Session, Open Session, Save Session, Save Session As…) and Settings (Manage Presets… | Appearance: System / Light / Dark).
...
- **Audit trail**: `EditVersionDialog` and `EditIterationDialog` both split `*.notes` into user notes (editable) and system entries (lines starting with `[Reverted`, `[Promoted`, `[Sent Output`, `[Deprecated`, or legacy `[REVERTED`). System entries are shown in a read-only **"Log"** `CTkTextbox` (`state="disabled"`) and are always appended unchanged after user notes on save. Module-level `_SYSTEM_NOTE_PREFIXES = ("[Reverted", "[Promoted", "[Sent Output", "[Deprecated", "[REVERTED")` is the shared constant. Note formats: `[Reverted to WIP] from {status} on {date} by {user} — {reason}`, `[Promoted to Production] on {date} by {user} — Runs: {run_list}`, `[Sent Output] on {date} by {user} — To: {to} — Subject: {subject}`, and `[Deprecated] on {date} by {user} — {reason}`. **Frame-level audit tables**: `VersionFrame` and `IterationFrame` also render system notes as a read-only `ttk.Treeview` table (`height=5`, 4 columns for version: Event/Date/By/Details; 5 for iteration: +Runs). The vertical `CTkScrollbar` is always shown (no entry-count guard). Treeview uses `sticky="nsew"` so it fills its grid cell, preventing a visual gap when the always-present scrollbar is taller than the 5-row treeview. Columns are autofitted via `autofit_tree_columns()`; double-clicking a row opens `show_audit_detail_popup` — a resizable `CTkToplevel` with the Details field in a wrapping `CTkTextbox`. All shared helpers (`AUDIT_NOTE_PREFIXES`, `parse_audit_note`, `parse_audit_note_extended`, `autofit_tree_columns`, `show_audit_detail_popup`, `show_comm_detail_popup`) live in `theme.py`.
...
- **User presets**: `app/core/settings.py` owns `AppSettings` (dataclass: `project_presets: dict[str, list[dict[str, str]]]` where each entry is `{"name": str, "id": str}`) and `SettingsManager` (load/save/`entity_id_for`/`add_preset_entry`/`merge_from_file`). A module-level lazy singleton `get_settings_manager()` is used throughout; eagerly called in `MainWindow.__init__`. Persisted to `~/Documents/FEA_Trace/settings.json`; old plain-string entries are silently promoted on load. `NewEntityDialog` and `EditEntityDialog` use `CTkComboBox` for Project Code and Entity Name; `_update_id_from_name` checks `entity_id_for()` before auto-generating. On confirm, if either value is new the user is prompted to save to presets. **Settings → Manage Presets…** opens `ManagePresetsDialog`: two-panel `CTkToplevel` (project codes left, name+ID entries right); edit via `_ask_value()` (single field) for project codes and `_ask_name_and_id()` (combined two-field dialog) for entries; double-click also triggers edit; Import from file… does an additive merge.
- **Send output / communication log**: `COMMUNICATIONS_FOLDER = "05_Communications"` constant in `app/config.py`. `CommunicationRecord` (fields: `sent_at`, `sent_by`, `to`, `subject`, `body_summary`, `eml_filenames: list[str]`, `run_refs: list[str]`) defined in `schema.py` after `ArtifactRecord`. `VersionRecord` gains `communications: list[CommunicationRecord]`. `FEAProject.add_communication(version_id, record, version_note, iter_notes)` appends the record, writes audit notes to version and referenced iterations, then calls `_write()`. `SendOutputDialog` (`app/gui/dialogs/send_output_dialog.py`): run-selection checkboxes (WIP runs disabled), Subject field (pre-filled), read-only auto-generated body (regenerates on checkbox/subject change via `_on_selection_change`), **"Open Draft in Outlook"** opens `mailto:?subject=…` and simultaneously places the body as `CF_HTML` (`<pre style="font-family: Courier New">`) on the Windows clipboard via `ctypes` — Ctrl+V in Outlook pastes in Courier New (works with both Classic and New Outlook; no extra dependency). A **Copy** button (`copy.png`) in the body header provides the same clipboard action on demand. **No To field in UI** — recipients are added in Outlook; `_to_var` is populated from `.eml` import only. Body format: `=`×65 borders, colon-aligned header fields with multi-line continuation indent, optional `--- Step files ---` section (scans `01_Source/{version_id}/` on disk), `--- Reported Results ---` per iteration with `[STATUS]`, last-run date, description, and selected run numbers. `.eml` import via Python `email` module, collision-safe copy to `05_Communications/`. **Save Record** enabled when ≥ 1 run selected + Subject non-empty + `.eml` imported. VersionFrame action bar: **"Send Output"** button → `_on_send_output()` → opens dialog, reloads on `dlg.result`. EntityFrame row 4: `_comms_section` (`ttk.Treeview`, 6 columns: Date/By/Version/To/Subject/.eml, hidden when no communications) with full sort/filter/search support — left-click heading to sort (▲/▼), right-click for filter popup (⊿); To column is multi-value (`_COMMS_MULTI_VALUE_COLS`): comma-separated recipients split into individual filter options; Subject and .eml suppressed from filter (`_COMMS_NO_FILTER_COLS`); Date column has ↓/↑ Newest/Oldest toggle; a `_search_comms_var: ctk.StringVar` drives a real-time "Search:" bar (label + `CTkEntry` + ✕) above the table, triggering `_refresh_comms()` on write — combined AND with column filters; state resets on `load()`. Double-clicking any comms row opens `show_comm_detail_popup` (from `theme.py`): static fields + per-file Open/Restore buttons + **"+ Add .eml"**; callbacks `on_add_eml(dest)` and `on_files_changed()` persist changes. `show_comm_detail_popup` uses `popup.geometry("520x1")` + plain `tk.Frame` container + `container.winfo_reqheight()` for reliable auto-sizing.
- **Version source files**: `SOURCE_FOLDER = "01_Source"` in `app/config.py`. `SourceComponentRecord`
  (fields: `entity_path`, `entity_name`, `project_code`, `version_id`, `copied_files: list[str]`)
  defined in `schema.py` before `RunRecord`. `VersionRecord` gains `source_components:
  list[SourceComponentRecord]`. `FEAProject.copy_version_source_files(version_id, step_files)`
  copies files into `01_Source/{version_id}/` (collision-safe). `FEAProject.get_version_source_folder(version_id)`
  returns the path without creating it. `add_version()` and `update_version_metadata()` accept
  `source_components=None`. `SelectSourceVersionDialog` uses `apply_sidebar_style()` +
  `style="Sidebar.Treeview"`; checkbox multi-select via ☐ / ☑ Unicode in node text; one version per
  entity enforced; result is `list[(entity_path, entity_name, project_code, version_id, step_files)]`
  or `None`. `EditVersionDialog` pre-fills `_file_entries` from `version.source_components`; entries
  with `path=None` are pre-existing (not re-copied); only entries with a real `Path` are passed to
  `copy_version_source_files()`. Assembly Components `ttk.Treeview` uses
