...
**Schema history:** `1.0.0` had a `RepresentationRecord` level between Version and Iteration. `2.0.0` merges it into Iteration. `2.1.0` removes the redundant `design_changes` field from `IterationRecord`. `2.2.0` renames `VersionRecord.intent` → `description`. `2.3.0` adds `promoted_at: str` to `VersionRecord`. `2.4.0` (current) adds `status: IterationStatus` and `notes: list[str]` to `IterationRecord`. All migrations `1.0.0 → 2.4.0` are registered in `app/core/migration.py`; major migrations require user confirmation, minor ones are auto-applied.

### Status State Machines

**Iteration:** WIP ↔ DEPRECATED. Reverting DEPRECATED → WIP requires a reason (captured via `RevertReasonDialog` with `entity_type="Iteration"`), appended to `IterationRecord.notes`. DEPRECATED iterations: Edit disabled, New Run disabled, excluded from Promote to Production dialog and its WIP-run guard. Sidebar node shows `● Deprecated` suffix when deprecated.
