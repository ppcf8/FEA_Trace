...
- **Run input file autocheck**: every time `RunFrame` loads or its state changes, the solver deck (`.fem` / `.rad` / `.xml`) is checked on disk inside `03_Runs/VxxIxx_Run_##/`. For non-production runs `_check_input_file()` (`models.py`) is used and the panel shows **"⚠ Input File Not Found"**; for production runs `_check_production_artifacts()` is used instead and the panel shows **"⚠ Production Artifact Warnings"**. `RunFrame._get_warnings()` selects the right check and title based on `is_production`. The warning panel title is dynamic via `self._warning_title_label`.
