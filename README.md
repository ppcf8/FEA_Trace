...
- **Status state machines** — WIP / PRODUCTION / DEPRECATED for versions and iterations; WIP / CONVERGED / DIVERGED / PARTIAL / ABORTED for runs; each iteration can be promoted to PRODUCTION independently; reverting a run to WIP automatically cascades to the parent iteration (and version) when they are PRODUCTION — each cascade requires a mandatory reason, and cancelling any prompt aborts the entire operation
...
- **Sidebar expand / collapse** — tree starts fully collapsed on load; `⊟` / `⊞` header buttons collapse or expand the full tree; right-click for a themed context menu (per-project, per-entity, or global)
...
- **Send output with communication log** — a **"Send Output"** button on the Version panel opens a compose dialog: select which runs to share (WIP runs excluded), edit the subject, and review the auto-generated body (structured plain-text report with step files, iteration status, and selected runs); **"Open Draft in Outlook"** opens a new compose window via `mailto:` and simultaneously copies the body to the clipboard as Courier New–formatted HTML — Ctrl+V in Outlook pastes with fixed-width formatting in both Classic and New Outlook; a **Copy** button in the body header provides the same action on demand; import a sent `.eml` to populate To/Subject and attach the file; saving stores a `CommunicationRecord` in the YAML, copies the `.eml` to `05_Communications/`, appends a `[Sent Output]` audit note to the version and each referenced iteration, and updates the EntityFrame Communications panel; the Communications table supports column sorting and per-column filtering (To column splits comma-separated recipients into individual filter values); additional `.eml` files can be attached later via the Communication Detail popup
